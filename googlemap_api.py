...
# Global cache for the graph and its spatial index (KD-Tree)
graphs_cache = {}
spatial_index = {"tree": None, "nodes": []}

def get_smooth_path(G, nodes):
    """ Extracts high-resolution geometry for map rendering """
    if not nodes or len(nodes) < 2: return None
    path_coords = []
    for i in range(len(nodes) - 1):
        u, v = nodes[i], nodes[i + 1]
...
def get_or_build_graph(date_str, hour_val):
    """ Loads graph and builds a KD-Tree for instant nearest-neighbor search """
    global graphs_cache, spatial_index
    cache_key = f"{date_str}_{hour_val}"
    
    if cache_key in graphs_cache:
        return graphs_cache[cache_key]
    
    # Memory management for free-tier hosting
    graphs_cache.clear()
    
    file_path = f'graphs/graph_{hour_val}.pkl'
    if os.path.exists(file_path):
        print(f"Loading graph for {hour_val}:00 and building spatial index...")
        with open(file_path, 'rb') as f:
            G = pickle.load(f)
        
        # Build KD-Tree for O(log N) nearest neighbor lookup
        nodes_list = list(G.nodes)
        coords = np.array([(n[0], n[1]) for n in nodes_list]) # [lng, lat]
        spatial_index["tree"] = KDTree(coords)
        spatial_index["nodes"] = nodes_list
...
def calculate_route(request: RouteRequest):
    G = get_or_build_graph(request.date_str, request.hour_val)
    if not G.nodes or spatial_index["tree"] is None:
        return {"shade_route": [], "standard_route": [], "stats": {"shade": {"dist": 0, "shadow": 0}, "standard": {"dist": 0, "shadow": 0}}}

    # Fast nearest node lookup using the KD-Tree
    _, start_idx = spatial_index["tree"].query([request.start_lng, request.start_lat])
    _, end_idx = spatial_index["tree"].query([request.end_lng, request.end_lat])
    
    start_node = spatial_index["nodes"][start_idx]
    end_node = spatial_index["nodes"][end_idx]

    # 1. Standard shortest path (by distance)
    standard_nodes = nx.shortest_path(G, source=start_node, target=end_node, weight='length')
    dist_standard = sum(G.get_edge_data(standard_nodes[i], standard_nodes[i+1]).get('length', 0) for i in range(len(standard_nodes)-1))

    # 2. Shady path (Dynamic weighting: distance + sun penalty)
    NEW_SUN_PENALTY = 3.0  
    TRANSITION_COST = 5.0
...
if __name__ == "__main__":
    import uvicorn
    # Pre-load one graph to warm up the server
    get_or_build_graph('2025-06-21', 14)
    uvicorn.run(app, host="0.0.0.0", port=8000)
